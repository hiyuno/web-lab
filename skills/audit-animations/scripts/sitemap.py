...
found on the home page, so pages missing from the sitemap still show up. Same approach as
audit-seo/scripts/sitemap.py, kept as its own copy so this skill installs independently.
...
UA = {"User-Agent": "Mozilla/5.0 animation-web-audit"}
...
def main():
    ...
    site = site.rstrip("/") + "/"
    pages = {}
    had_sitemap = bool(read_sitemap(urllib.parse.urljoin(site, "sitemap.xml")))
    ...
    if a.json:
        print(json.dumps({"site": site, "has_sitemap": had_sitemap, "pages": rows}, indent=2, ensure_ascii=False))
    else:
        print("| # | slug | url | source |\n|---|---|---|---|")
        for i, r in enumerate(rows, 1):
            print(f"| {i} | {r['slug']} | {r['url']} | {r['source']} |")
        print(f"\n{len(rows)} pages, sitemap.xml {'found' if had_sitemap else 'MISSING'}", file=sys.stderr)

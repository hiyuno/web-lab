...
import { typeScale, TEXT_STEPS } from '../engine/tokens'
...
export function Typography({ k, copy }: { k: Knobs; copy: Copy }) {
  ...
  return (
    <Pattern n={1} title="Typography" note={`${k.font} · ratio ${k.ratio} · base ${k.base}px`}>
      <div className="stack">
        <h1>{copy.title}</h1>
...
export function Color({ ramps, sem, pairs, pinned }: { ramps: Ramps; sem: Semantic; pairs: PairResult[]; pinned: number | null }) {
  ...
  return (
    <Pattern n={2} title="Color" note={`accent ${pinned ? `pinned on ${pinned}` : 'snapped to the ramp'} · ${fails ? `${fails} pair(s) fail` : 'all pairs pass'}`}>
      <div className="stack">
...
export function Buttons({ copy }: { copy: Copy }) {
  ...
  return (
    <Pattern n={3} title="Buttons" note="4 variants · 3 sizes · 9 states · scale 0.96 on press">
      <div className="stack">
...
export function Cards({ k, copy }: { k: Knobs; copy: Copy }) {
  return (
    <Pattern n={4} title="Cards" note={`${k.corner} corners · radius ${k.radius}px · ${k.shadow} shadow · border ${k.border}px`}>
      <div className="grid">
...
export function Form() {
  ...
  return (
    <Pattern n={5} title="Form" note="labels visible · errors next to the field · 24px targets">
...
export function Navigation({ copy }: { copy: Copy }) {
  return (
    <Pattern n={6} title="Navigation" note="header · menu · footer · current page in accent">
...
export function Hero({ copy }: { copy: Copy }) {
  return (
    <Pattern n={7} title="Hero" note="density · content width · image treatment">
...
export function List() {
  ...
  return (
    <Pattern n={8} title="List" note="rows · hover · status tags · long content">
      <div className="list">
...
export function Feedback() {
  return (
    <Pattern n={9} title="Feedback" note="alert · toast · tags · empty · loading">
      <div className="grid">
        <div className="stack">
...
export function Dialog({ k, reduced }: { k: Knobs; reduced: boolean }) {
  ...
  return (
    <Pattern n={10} title="Dialog and motion" note={`${k.duration} ms · ease ${k.ease} · ${reduced ? 'reduced motion on' : 'interruptible transitions'}`}>
      <div className="stack">

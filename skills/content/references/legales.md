# Legales y privacidad · [proyecto]

Fecha: [aaaa-mm-dd] · Autores: Rosenfeld con Schneier · Estado: borrador | revisado por Schneier | aprobado

Nunca copiados de otro sitio. Datos reales del responsable, en lenguaje llano. Ley aplicable en
México: LFPDPPP publicada el 20 de marzo de 2025 (sustituye a la de 2010; la autoridad es la
Secretaría Anticorrupción y Buen Gobierno, no el INAI). Resumen en
`skills/security/references/legal-mx.md`. Si hay usuarios en Europa, Schneier añade lo que pide GDPR.

## Datos del responsable (los da el usuario)

- Razón social o nombre: [ ]
- Domicilio: [ ]
- Correo para derechos ARCO y privacidad: [ ]
- Datos personales que se recogen, por formulario: [ ] (de los briefs y wireframes)
- Finalidades primarias (necesarias) y secundarias (opcionales): [ ]
- Con quién se comparten (hosting, analítica, correo, pagos, CRM): [ ]
- Tiempo de conservación: [ ]
- Cookies no esenciales: [ninguna | cuáles y para qué]

## Aviso de privacidad (LFPDPPP 2025) · contenido mínimo

- [ ] Identidad y domicilio del responsable
- [ ] Datos personales que se tratan, identificando cuáles son sensibles (obligatorio en la ley de 2025)
- [ ] Finalidades primarias y secundarias, con forma de negarse a las secundarias
- [ ] Transferencias a terceros y su finalidad (ya no obligatorio en la ley de 2025; se mantiene como buena práctica y por GDPR)
- [ ] Medios para ejercer derechos ARCO (acceso, rectificación, cancelación, oposición) y para revocar el consentimiento
- [ ] Opciones para limitar uso o divulgación
- [ ] Uso de cookies y tecnologías de rastreo, si las hay
- [ ] Cómo se comunican cambios al aviso
- [ ] Fecha de última actualización
- [ ] Aviso simplificado junto a cada formulario: quién, para qué, enlace al integral

## Términos y condiciones (si hay venta o cuenta)

- [ ] Qué se ofrece, precios e impuestos, formas de pago
- [ ] Entrega, cancelación, devoluciones, garantía
- [ ] Cuenta de usuario: registro, responsabilidad de la contraseña, baja
- [ ] Contenido de usuarios: qué se permite, qué se retira
- [ ] Propiedad intelectual, limitación de responsabilidad, ley aplicable
- [ ] Contacto y fecha

## Política de cookies (solo si hay no esenciales)

- [ ] Qué cookies, de quién, para qué, cuánto duran
- [ ] Cómo aceptar, rechazar y cambiar de opinión
- [ ] Banner: texto en lenguaje llano, rechazar tan visible como aceptar, nada se carga antes del consentimiento

## Textos que revisa Schneier

| Texto | Dónde | Regla |
|-------|-------|-------|
| Error de login | /login | genérico, no revela si el correo existe |
| Recuperación de contraseña | /recuperar | "Si el correo está registrado, recibirás un enlace" |
| Banner de cookies | global | honesto, rechazar visible |
| Aviso simplificado | cada formulario | quién, para qué, enlace |
| Baja de cuenta | /cuenta/baja | qué se borra y cuándo |

## Veredicto de Schneier

[aprobado | con condiciones | bloqueado] · [razón] · [fecha]
